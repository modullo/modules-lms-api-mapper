Vue.component("module-api-mapper-form", {
    props: {
        modulesMap: {
            default: () => [],
        },
    ...
    template: `
        <div class="container">
            <form class="form" @submit.prevent="validateBeforeSubmit">
                <div class="form-row">
                    <div class="form-group col-lg-6" v-for="(map,mapKey) in modulesMap" v-if="map.visible">
    ...
    data() {
        return {
            form: this.modulesMapData,
        };
    },
    mounted() {
        console.log(this.modulesMapData)
    },
    watch: {
    ...
    methods: {
        validateBeforeSubmit(ev) {
            ...
            axios.post(`/tenant/api-mapper?update-type=module`, this.form).then(res => {
                // ev.target.reset()
                console.log(res.data)
                this.$emit('update-module-map',this.form)
                loader.hide();
